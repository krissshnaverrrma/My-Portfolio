...
class SystemValidator:
    ...
    def register_commands(app):
        ...
        def run_tests(extra_args):
            ...
            try:
                ...
                print(
                    f"{RED}❌ Pytest is not Installed. Run 'pip install pytest' first.{END}")
                sys.exit(1)
            os.environ["FLASK_TESTING"] = "true"
            validator = SystemValidator(app)
            exit_code = validator.run_pytest_quietly(extra_args)
            sys.exit(exit_code)
    ...
    def run_pytest_quietly(self, extra_args):
        ...
        RETURNS the exit code instead of calling sys.exit().
        """
