...
def contact():
    ...
    if request.method == 'POST':
        name, email, subject, message = request.form.get('name'), request.form.get(
            'email'), request.form.get('subject'), request.form.get('message')
        if name and email and message:
            if save_contact_message(name, email, subject, message):
                success = True
        else:
            flash("Please fill in required fields.", "warning")
    return render_template('contact.html', success=success, profile=get_user_profile())
